...
mob_test <- comb_new %>% 
  ...
  theme_bw()+
  labs(x = "Year starting from 1990", y = "Mobile Phone Subscription Rate")
ggplotly(mob_test)
```
...
```{r testplot-landline, fig.cap="Fitting linear regression for telephone subsripitions in each continent"}
land_test <- comb_new %>% 
  ...
  facet_wrap(~ continent, scales = 'free')+
  theme_bw()+
  labs(x = "GDP per Capita", y = "Land-line Subscription Rate")
ggplotly(land_test)
```
...
Meanwhile, the other three variables provided in the data set are also important factors in explaining the variation of landline subscriptions. Thus, here has constructed a linear regression model for `land-line subscription`:
 ...
 $$\widehat{landline-subs} = 11.408  + 0.0006~gdp-per-cap + 0.139~mobilesubs - 0.6326~year1990$$ 
...
resid_panel(fit_land, plots = c("resid","qq","cookd","lev"), theme = "bw", smoother = TRUE)+
  labs(title = "Residual Panels for Diagnostic")
```
