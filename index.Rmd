...
a <- ggplot(new_data, aes(x = year, y = mean_subscription, color = type)) + geom_point() + geom_smooth()+
  ...
  scale_y_continuous(breaks = seq(0, 100, 10)) + labs(y = "average subcription of 100 people")
...
knitr::kable(gdp, format.args = list(big.mark = ","), caption = "Average GDP per capita on each continent")
```
...
```{r}
 b <- ggplot(europe, aes(x = year, y = mean_subscription, color = type)) + geom_point() + geom_smooth()+
  scale_x_continuous(breaks = seq(1990, 2018, 2)) +
  scale_y_continuous(breaks = seq(0, 100, 10)) + labs(y = "average subcription of 100 people")
```
