...
## Discovery of any possible relationship between mobile phone subscriptions and provided factors.
...
### Obtaining a scatter plot matrix to see if any associations between both mobile phone and telephone subscription and other variables.
...
### Test to fit the linear regression and Local regression models on scatter plots for each continent
...
This linear model contains all the significant variables to explain the variation of landline subscription, yet the statistics suggest these may not be the suitable ones. Moreover, these estimate coefficients of each variable in the model are very small. Although, the p-value suggests the all are the important factors yet the effect and help on explaining and predicting the changes in landline subscription is limited or minor. Hence, understanding the variation of landline subscription rate requires additional relevant information and for constructing a possible prediction model.
...
"?? 首先用linear还有LOESS来判断可以用linear。
然后尝试要不要把continent加入到linear model里面去，因为不同continent的 mobile phone subscriptions 不同。??"

!!! Use of conditioning plots (Coplots)? for time factor (i.e. with in the same year? link it to the new question 1)

### Diagnostics on the linear fitting model
